<?php
    ...
    class EmployeeController extends Controller
    {
        ...
        public function show($id)
        {
            if (\Auth::user()->can('Show Employee')) {
                ...
                // Ensure employees can only view their own details
                if (\Auth::user()->type === 'employee' && $employee->user_id !== \Auth::id()) {
                    return redirect()->back()->with('error', __('Permission denied.'));
                }

                // Set default approval_status if not set
                if (empty($employee->approval_status)) {
    ...
    /**
     * Approve employee details
     */
    public function approve($id)
    {
        // Allow company and HR users to approve
        if (\Auth::user()->type !== 'company' && \Auth::user()->type !== 'hr') {
            if (!\Auth::user()->can('approve employee')) {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        }
        ...
        EmployeeNotification::create([
            'employee_id' => $employee->id,
            'type' => 'approved',
            'message' => __('Your employee details have been approved.'),
            'created_by' => \Auth::id(),
            'seen' => false,
        ]);

        return redirect()->back()->with('success', __('Employee details approved successfully.'));
    }
    ...
    {
        // Allow company and HR users to reject
        if (\Auth::user()->type !== 'company' && \Auth::user()->type !== 'hr') {
            if (!\Auth::user()->can('approve employee')) {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        ...
        EmployeeNotification::create([
            'employee_id' => $employee->id,
            'type' => 'rejected',
            'message' => __('Your employee details have been rejected. Please review and update.'),
            'rejection_reason' => $request->rejection_reason,
            'created_by' => \Auth::id(),
            'seen' => false,
        ]);

        return redirect()->back()->with('success', __('Employee details rejected.'));
    }

    /**
     * Request approval again after rejection
     */
    public function requestApproval($id)
    {
        $employee = Employee::findOrFail($id);

        // Only allow employee to request approval for their own details
        if (\Auth::user()->type !== 'employee' || $employee->user_id !== \Auth::id()) {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        ...
        return redirect()->back()->with('success', __('Approval request submitted. Waiting for review.'));
    }
    ...
    {
        $notification = EmployeeNotification::findOrFail($id);
        
        // Only allow the employee to mark their own notifications as seen
        if ($notification->employee->user_id !== \Auth::id()) {
