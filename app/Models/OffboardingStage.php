...
class OffboardingStage extends Model
{
    protected $fillable = [
        'title',
        'order',
    ...
    {
        $process = OffboardingProcess::where('created_by', \Auth::user()->creatorId())
            ->where('stage', $this->id);
        ...
        $process = $process->with(['employee.user', 'resignation', 'termination'])->orderBy('order')->get();
