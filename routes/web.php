...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/get-departments-by-branch/{id}', [ProjectController::class, 'getDepartmentsByBranch'])->name('get-departments-by-branch');

    Route::post('/get-departments-by-id', [ProjectController::class, 'getDepartmentsById'])->name('get-departments-by-id');
});
...
// routes/web.php
Route::get('/get-project-employees/{projectId}', [TimeSheetController::class, 'getProjectEmployees'])->name('project.employees');
...
Route::group(['middleware' => ['verified']], function () {
    ...
    // Approval routes
    Route::post('employee/{id}/approve', [EmployeeController::class, 'approve'])->name('employee.approve')->middleware(['auth', 'XSS']);
    Route::post('employee/{id}/reject', [EmployeeController::class, 'reject'])->name('employee.reject')->middleware(['auth', 'XSS']);
    Route::post('employee/{id}/request-approval', [EmployeeController::class, 'requestApproval'])->name('employee.request-approval')->middleware(['auth', 'XSS']);
    Route::post('employee-notification/{id}/mark-seen', [EmployeeController::class, 'markNotificationSeen'])->name('employee.notification.mark-seen')->middleware(['auth', 'XSS']);
    ...
    // Route::get('/leave/action/{id}/{status}', [LeaveController::class, 'action'])->name('leave.action'); // Commented out - duplicate route, causes conflict
    ...
    // Notification routes
    Route::post('/notifications/{id}/read', [App\Http\Controllers\NotificationController::class, 'markAsRead'])->middleware(['auth'])->name('notifications.read');
    Route::post('/notifications/{id}/delete', [App\Http\Controllers\NotificationController::class, 'delete'])->middleware(['auth'])->name('notifications.delete');
    Route::post('/notifications/mark-all-read', [App\Http\Controllers\NotificationController::class, 'markAllAsRead'])->middleware(['auth'])->name('notifications.markAllRead');
